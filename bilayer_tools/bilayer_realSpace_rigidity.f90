...
do while(.not. is_end)
    ...
    do while(id1<=cnfg%n-1)
        ...
        elseif(cnfg%tp(id1) == patts%patts(2)%patt(1)) then
            ...
            n1 = get_direction(cnfg%r(:,id1), cnfg%r(:,id1+patts%patts(2)%n-1), cnfg%Lbox)
        ...
        do while(id2<=cnfg%n)
            ...
            elseif(cnfg%tp(id2) == patts%patts(2)%patt(1)) then
                ...
                n2 = get_direction(cnfg%r(:,id2), cnfg%r(:,id2+patts%patts(2)%n-1), cnfg%Lbox)
...
open(newunit=io,file=trim(charus)//"/neighbots.data")
...
open(newunit=io, file=trim(charus)//"/PSi.data")
